...
function getLetters() {
    try {
        ...
        Reports.Getbooking( managment, fromDate, toDate, function (val) {
            fillPoints(val[0]);
        });
    } catch (err) {
        alert(err);
...
function fillPoints(data) {
    try {
        ...
        if (data != "") {
            ...
            if (files.length > 0) {
                $('#tblStudentPoints').remove();
                createTable("tblStudentPoints", "divStudentPoints", ["#", "القاعة", "البرنامج", "التاريخ","عدد الايام","من الساعة","الى الساعة","مقدم الحجز"]);
...
function appendFileTR(fileDetails, i) {
    try {
       // alert(fileDetails.week);
        ...
        $(tr).append('<td><span id="lblStage_Name" class="index">' + fileDetails.managmentname + '</span>');
        $(tr).append('<td><span id="lblLesson_Name" class="index">' + fileDetails.program_name+ '</span>');
        $(tr).append('<td><span id="lblStudent_Name" class="index">' + fileDetails.create_date + '</span>');
 $(tr).append('<td><span id="lblLesson_Name" class="index">' + fileDetails.days_no+ '</span>');
        $(tr).append('<td><span id="lblStudent_Name" class="index">' + fileDetails.from_hour + '</span>');
        $(tr).append('<td><span id="lblStudent_Name" class="index">' + fileDetails.to_hour + '</span>');
       $(tr).append('<td><span id="lblStudent_Name" class="index">' + fileDetails.customer_name+ '</span>');
