...
function getContacts() {
    try {
        var name = $("#ddlSupervisor option:selected").text();
        $("#reportDiv").show();
        $('#tblStudentPoints').remove();
        Reports.GetContacts("", function (val) {
...
function fillPoints(data) {
    try {
        ...
        if (data != "") {
            ...
            if (files.length > 0) {
                $('#tblStudentPoints').remove();
                createTable("tblStudentPoints", "divStudentPoints", ["#", "الاسم", "التليفون", "البريد ","العنوان"]);
...
function appendFileTR(fileDetails, i) {
    try {
       // alert(fileDetails.week);
        ...
        $(tr).append('<td><span id="lblLesson_Name" class="index">' + fileDetails.name + '</span>');
        $(tr).append('<td><span id="lblStage_Name" class="index">' + fileDetails.phone1 + '</span>');
        $(tr).append('<td><span id="lblStudent_Name" class="index">' + fileDetails.email + '</span>');
        $(tr).append('<td><span id="lblStudent_Name" class="index">' + fileDetails.address + '</span>');
       // $(tr).append('<td><span id="lblStudent_Name" class="index">' + fileDetails.email + '</span>');
