...
var deleteWebServiceMethod = "archived_coursesCls.asmx/Delete";
// global variable used in row_click and update functions
var editWebServiceMethod = "archived_coursesCls.asmx/Edit";
...
//                var Id = "";
//                $("#SavedivLoader").show();
//                archived_coursesCls.Save(Id, basicData, function (val) {
//                    if (val === true) {
...
<div class="block">
                        <div class="block-title" style="background:#000">
                            <h5><a href="courseDetails_archived.aspx?code=${element.code}">${element.name}</a></h5>
                        </div>
...
function drawAllCourses() {
    try {
        ...
        archived_coursesCls.get_Courses("", "", function (val) {
            //debugger
            var data = "";
...
function drawCourses(filter){
    try {
        ...
        archived_coursesCls.get_Courses(filter, "", function (val) {
            //debugger
            var data = "";
            if (val[0] == "0") {
...
function searchCourses() {
    try {
        ...
        archived_coursesCls.get_Courses("", courseName, function (val) {
            var data = "";
            var arr1 = JSON.parse(val[1]);
            CoursesList = arr1;
            numPages = Math.ceil(CoursesList.length / records_per_page);
