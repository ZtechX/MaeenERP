...
function onUploadFileComplete(sender, args) {
    try {
        ...
        var folders = "Hall_Module/Images"
...
function deleteUploadedFile(val) {
    try {
        MultiFileUploader.DeleteFile(val, "Work_Module", onSuccessDeletedFile);
    } catch (err) {
        alert(err);
    }
}
    ...
    try {
        ...
        if ($('.photoWrapNew img').length > 0) {
            if ($('#tblUploadedFilesNew').length == 0) {
                createTblUploadedFilesNew(["Index", "File", "Name", "Main Image", "Delete"]);
            }
