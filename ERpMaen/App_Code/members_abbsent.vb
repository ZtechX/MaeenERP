...
#Region "save_file"
    ...
    End Function
#End Region
#Region "get_member_session"
    ''' <summary>
    ''' get  Type data from db when update
    ''' </summary>
    <WebMethod()>
    <System.Web.Script.Services.ScriptMethod()>
    Public Function get_member_session(ByVal session As String) As String()
        Dim dt_members As DataTable
        Dim dt_user As DataTable
        Dim dt_session As DataTable
        Dim dt As DataTable
        Dim Names As New List(Of String)(10)
        Try
            dt_user = DBManager.Getdatatable("select * from tblUsers where id=" + LoginInfo.GetUserCode(Context.Request.Cookies("UserInfo")).ToString())
            dt_members = DBManager.Getdatatable("SELECT * from tblMembers  where  comp_id= " + dt_user.Rows(0).Item("comp_id").ToString())
            dt_session = DBManager.Getdatatable("SELECT * from tblSession_abssent  where  comp_id= " + dt_user.Rows(0).Item("comp_id").ToString() + " and session_id=" + session.ToString)
            If dt_session.Rows.Count = 0 Then
                If dt_members.Rows.Count <> 0 Then
                    ...
                    For Each row2 As DataRow In dt_members.Rows
                        ...
                        dict.Add("session_id", session.ToString)
                        dict.Add("comp_id", dt_user.Rows(0).Item("comp_id").ToString)
                        dict.Add("abbsent", 0)
                        PublicFunctions.TransUpdateInsert(dict, "tblSession_abssent", id, _sqlconn, _sqltrans)
                    Next
                End If
            End If
            dt = DBManager.Getdatatable("SELECT  tblMembers.id as 'member_id',* from tblSession_abssent join tblMembers on tblSession_abssent.member_id=tblMembers.id   where  tblSession_abssent.session_id=" + session.ToString)
...
#Region "get_board_session"
    ...
    <WebMethod()>
    <System.Web.Script.Services.ScriptMethod()>
    Public Function get_board_session(ByVal boards As String) As String()
        Dim dt As DataTable
        Dim Names As New List(Of String)(10)
        Try
            dt = DBManager.Getdatatable("SELECT * from tblSessions   where  board_id=" + boards.ToString)
            If dt.Rows.Count <> 0 Then
...
#Region "abbsent"
    ...
    <WebMethod()>
    <System.Web.Script.Services.ScriptMethod()>
    Public Function abbsent(ByVal basicDataJson As List(Of Object), ByVal session As String) As Boolean
        Dim dt_members As DataTable
        Dim dt_user As DataTable
        Dim dt As DataTable
        Dim Names As New List(Of String)(10)
        Try
            ...
            For Each DetailsJSON As Dictionary(Of String, Object) In basicDataJson
                ...
                Dim dict = New Dictionary(Of String, Object)
                dt_members = DBManager.Getdatatable("SELECT * from tblSession_abssent  where  comp_id= " + dt_user.Rows(0).Item("comp_id").ToString() + " and session_id=" + session.ToString + " and member_id=" + dictBasicDataJson("member_id").ToString)
...
#Region "disapproved"
    ...
    <WebMethod()>
    <System.Web.Script.Services.ScriptMethod()>
    Public Function disapproved(ByVal basicDataJson As List(Of Object), ByVal session As String) As Boolean
        Dim dt_members As DataTable
        Dim dt_user As DataTable
        Dim dt As DataTable
        Dim Names As New List(Of String)(10)
