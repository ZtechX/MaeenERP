...
Public Class out_inside_letters
...
#Region "Save"
    ...
    Public Function Save(ByVal id As String, ByVal basicDataJson As Object, ByVal attch_file_DataJsonList As List(Of Object)) As Boolean
        Try
            ...
            dictBasicDataJson("parcode") = getBarCode()

            If PublicFunctions.TransUpdateInsert(dictBasicDataJson, "tblletter", id, _sqlconn, _sqltrans) Then
                Dim letter_id
                If id <> "" Then
                    letter_id = id
                Else
...
#Region "Edit"
    ...
    Public Function Edit(ByVal editItemId As String) As String()
        ...
        Try
            Dim str As String = PublicFunctions.GetDataForUpdate("tblletter", editItemId)
            Names.Add("1")
            Names.Add(str)

            Dim dtattch_file As New DataTable
            Dim str1 As String = ""
            dtattch_file = DBManager.Getdatatable("Select * from tblImages where Source='out_letter' and isnull(deleted,0)=0  and Source_id=" + editItemId.ToString())
...
#Region "Delete"
    ...
    Public Function Delete(ByVal deleteItem As String) As String()
        Dim Names As New List(Of String)(10)
        Try
            If PublicFunctions.DeleteFromTable(deleteItem, "tblletter") Then
                DBManager.ExcuteQuery("delete from tblImages where Source_id=" + deleteItem.ToString + " and Source='out_letter' ")
                Names.Add("1")
...
#Region "letter_replay"
    ''' <summary>
    ''' get  Type data from db when update
    ''' </summary>
    <WebMethod()>
    <System.Web.Script.Services.ScriptMethod()>
    Public Function letter_replay(ByVal editItemId As String) As String()
        Dim unitsId = editItemId
        Dim Names As New List(Of String)(10)
        Try
            Dim str As String = PublicFunctions.GetDataForUpdate("tblletter", editItemId)
...
#Region "get_fromdep"
    ...
    <WebMethod()>
    <System.Web.Script.Services.ScriptMethod()>
    Public Function get_fromdep(ByVal dep_type As String) As String()
        ...
        Try
            ...
            If dep_type = 1 Then
                dtattch_file = DBManager.Getdatatable(" select id as'id',description as 'name' from tbllock_up where type='MG'")

            ElseIf dep_type = 2 Then
                dtattch_file = DBManager.Getdatatable(" select id as'id',name_ar as 'name' from tblcustomers where type_id !=0")
            ElseIf dep_type = 3 Then
                dtattch_file = DBManager.Getdatatable(" select id as'id',name_ar as 'name' from tblworker")
            End If
            ...
            If dtattch_file.Rows.Count <> 0 Then
                Names.Add("1")
                Dim x As Integer = 1
                While x < dtattch_file.Rows.Count + 1
                    Names.Add(dtattch_file(x - 1)("name").ToString() & "|" & dtattch_file(x - 1)("id").ToString())
                    x += 1
                End While
...
#End Region
   ...
   <System.Web.Script.Services.ScriptMethod()>
    Public Function getBarCode() As String
        ...
        Dim val = ""
        dt = DBManager.Getdatatable("SELECT isNull (parcode,'') parcode  FROM tblletter where id= (select max(id) from tblletter where type= 1)")
