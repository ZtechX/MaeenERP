...
#Region "khairllah"
    ...
    End Function
    Public Shared Function GenerateCodeOFunit1(ByRef _SQLConnection As SqlConnection, ByVal Purpose As String, ByVal Type As String, ByVal TableName As String, Optional ByVal GoToTemp As Boolean = False) As String

        Dim ftype As Char
        Dim fpurpose As Char
        ftype = Type.Substring(0, 1)
        fpurpose = Purpose.Substring(0, 1)
        Dim cls As New ClsCategoryType
        Dim GCode As String = fpurpose + ftype
    ...
    End Function
    Public Shared Function GenerateCodeOFunit(ByRef _SQLConnection As SqlClient.SqlConnection, ByRef _SQLTransaction As SqlClient.SqlTransaction, ByVal Purpose As String, ByVal type As String, ByVal TableName As String) As String
        ...
        fpurpose = Purpose.Substring(0, 1)
        Dim cls As New ClsCategoryType
        Dim GCode As String = fpurpose + ftype + "0"
        Dim strSql = "Select isnull('" + GCode + "'+convert(varchar,max(cast(substring(Code,3,10)as int))),'') " & _
               "from  " & TableName & "   where  substring (Code,1,3)='" + GCode + "' and len(Code)>= 7 "
        Dim sqlCommand As New SqlClient.SqlCommand(strSql, _SQLConnection, _SQLTransaction)
        Dim Result As String
        Try
            Result = sqlCommand.ExecuteScalar
        ...
        Else
            If Result.Length >= 7 Then
                If Result.Substring(3, Result.Length - 3) >= 9999 Then
                    Dim maxvalue As String = (CInt(Result.Substring(3, Result.Length - 3)) + 1).ToString
                    maxvalue = maxvalue + "000"
                    GCode = GCode + maxvalue
                Else
                    Dim maxvalue As String = (CInt(Result.Substring(3, Result.Length - 3)) + 1).ToString
                    maxvalue = maxvalue + "000"
                    GCode = GCode + maxvalue
